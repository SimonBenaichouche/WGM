...
def get_logger(filename, tqdm=False):
...
def get_trajectory_rk4(
    ...
    if pbar:  # Initialize progress bar only if pbar is True
        pbar_obj = tqdm(total=num_steps)
    else:
        pbar_obj = None
    ...
    update_iters = 300 #num_steps // tqdm_iters
    ...
    with tqdm(total=num_steps) as pbar:
        for t_step in range(num_steps):
            ...
            if t_step % update_iters == 0:
                ...
                if pbar_obj:  # Update the progress bar only if it's enabled
                    pbar_obj.set_description(desc)
                    pbar_obj.update(update_iters)
