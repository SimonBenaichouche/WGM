...
class res_cnn(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.cnn = nn.Sequential(PeriodicConv2d(in_channels, 64, kernel_size=5, padding=2),
                                nn.Tanh(),
                                PeriodicConv2d(64, 64, kernel_size=5, stride=1,padding =2),
                                nn.Tanh(),
                                PeriodicConv2d(64, 64, kernel_size=5, stride=1,padding=2),
                                nn.Tanh(),
                                PeriodicConv2d(64, 64, kernel_size=3, stride=1,padding=1),
                                nn.Tanh(),
                                PeriodicConv2d(64, 1, kernel_size=3, stride=1,padding=1))
...
class GradientNet(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.ipp_loss(predicted_gradient, samples, target_values,[mask1,mask2,mask3,mask4],sigma)
